...
class Trainer(object):
    def __init__(self,
        ...
        self.model.module = model.module
    ...
    def save_checkpoint(self, checkpoint_path):
        ...
        state_dict["model.module"] = self.model.module.state_dict()
    ...
    def load_checkpoint(self, checkpoint_path, load_only_params=False):
        ...
        Args:
            ...
            load_only_params (bool): Whether to load only model.module parameters.

        """
        state_dict = torch.load(checkpoint_path, map_location="cpu")
        self._load(state_dict["model.module"], self.model.module)
    ...
    def _load(self, states, model.module, force_load=True):
        model.module_states = model.module.state_dict()
        for key, val in states.items():
            try:
                if key not in model.module_states:
                    continue
                if isinstance(val, nn.Parameter):
                    val = val.data

                if val.shape != model.module_states[key].shape:
                    self.logger.info("%s does not have same shape" % key)
                    print(val.shape, model.module_states[key].shape)
                    if not force_load:
                        continue

                    min_shape = np.minimum(np.array(val.shape), np.array(model.module_states[key].shape))
                    slices = [slice(0, min_index) for min_index in min_shape]
                    model.module_states[key][slices].copy_(val[slices])
                else:
                    model.module_states[key].copy_(val)
            except:
                self.logger.info("not exist :%s" % key)
                print("not exist ", key)

    @staticmethod
    def get_gradient_norm(model.module):
        total_norm = 0
    ...
    def run(self, batch):
        ...
        mel_input_length = mel_input_length // (2 ** self.model.module.module.n_down)
        future_mask = self.model.module.get_future_mask(
            mel_input.size(2)//(2**self.model.module.n_down), unmask_future_steps=0).to(self.device)
        mel_mask = self.model.module.length_to_mask(mel_input_length)
        text_mask = self.model.module.length_to_mask(text_input_length)
        ppgs, s2s_pred, s2s_attn = self.model.module(
        ...
        loss.backward()
        torch.nn.utils.clip_grad_value_(self.model.module.parameters(), 5)
        self.optimizer.step()
        self.scheduler.step()
    ...
    def _eval_epoch(self):
        ...
        for eval_steps_per_epoch, batch in enumerate(tqdm(self.val_dataloader, desc="[eval]"), 1):
            batch = [b.to(self.device) for b in batch]
            text_input, text_input_length, mel_input, mel_input_length = batch
            mel_input_length = mel_input_length // (2 ** self.model.module.n_down)
            future_mask = self.model.module.get_future_mask(
                mel_input.size(2)//(2**self.model.module.n_down), unmask_future_steps=0).to(self.device)
            mel_mask = self.model.module.length_to_mask(mel_input_length)
            text_mask = self.model.module.length_to_mask(text_input_length)
